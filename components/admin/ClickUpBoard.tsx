...
export default function ClickUpBoard() {
    ...
    return (
        <div className="space-y-6">
            ...
            {showForm && (
                <form onSubmit={handleCreateTask} className="rounded-xl border border-[#f97316]/30 bg-[#f97316]/5 p-4 space-y-3">
                    <div className="flex gap-3 flex-wrap">
                        <input type="text" placeholder="Task name" value={newTask.name} onChange={e => setNewTask(p => ({ ...p, name: e.target.value }))} className="flex-1 min-w-[200px] bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/30 outline-none focus:border-[#f97316]/50" required />
                        <select value={newTask.listId} onChange={e => setNewTask(p => ({ ...p, listId: e.target.value }))} className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#f97316]/50">
                            {board.lists.map(l => (<option key={l.id} value={l.id}>{l.name}</option>))}
                        </select>
                    </div>
                    <input type="text" placeholder="Description (optional)" value={newTask.description} onChange={e => setNewTask(p => ({ ...p, description: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/30 outline-none focus:border-[#f97316]/50" />
                    <div className="flex gap-2">
                        <button type="submit" disabled={submitting} className="px-4 py-2 bg-[#f97316] text-black text-sm font-bold rounded-lg disabled:opacity-50">{submitting ? "Creating…" : "Create Task"}</button>
                        <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 border border-white/10 text-white/50 text-sm rounded-lg hover:text-white">Cancel</button>
                    </div>
                </form>
            )}
                    ...
                    return (
                        <div key={list.id} className="rounded-xl border border-white/8 bg-white/3 overflow-hidden">
                            <div className="flex items-center justify-between px-4 py-3 border-b border-white/8 bg-white/4">
                                ...
                                <span className="text-xs text-white/30 bg-white/5 rounded-full px-2 py-0.5">{tasks.length}</span>
                            </div>
                            <div className="p-2 space-y-2 max-h-[520px] overflow-y-auto">
                                {tasks.length === 0 && (<p className="text-center text-white/20 text-xs py-6">No tasks</p>)}
                                {tasks.map((task) => {
                                    ...
                                    return (
                                        <div key={task.id} className="rounded-lg border border-white/8 bg-[#0d1117] p-3 cursor-pointer hover:border-white/15 transition-all" onClick={() => setExpandedTask(isExpanded ? null : task.id)}>
                                            <div className="flex items-start justify-between gap-2">
                                                ...
                                                {task.priority && (
                                                    <span className={`text-[9px] font-bold uppercase tracking-wider border rounded-full px-1.5 py-0.5 whitespace-nowrap ${PRIORITY_STYLES[task.priority.priority] ?? PRIORITY_STYLES.normal}`}>{task.priority.priority}</span>
                                                )}
                                            </div>
                                            <div className="flex items-center flex-wrap gap-1.5 mt-2">
                                                <span className="text-[9px] font-bold uppercase tracking-wider rounded-full px-2 py-0.5" style={{ background: task.status.color + "22", color: task.status.color, border: `1px solid ${task.status.color}44` }}>{task.status.status}</span>
                                                {task.due_date && (<span className={`text-[10px] font-medium ${overdue ? "text-red-400" : "text-white/40"}`}>{overdue ? "⚠ " : ""}{formatDate(task.due_date)}</span>)}
                                                {task.tags.slice(0, 2).map(tag => (<span key={tag.name} className="text-[9px] font-bold uppercase tracking-wider rounded-full px-1.5 py-0.5" style={{ background: tag.tag_bg + "33", color: tag.tag_fg, border: `1px solid ${tag.tag_bg}55` }}>{tag.name}</span>))}
                                            </div>
                                                ...
                                                <div className="flex items-center gap-1 mt-2">
                                                    {task.assignees.slice(0, 3).map(a => (
                                                        a.profilePicture
                                                            ...
                                                            : <span key={a.id} className="w-5 h-5 rounded-full bg-[#f97316]/20 border border-[#f97316]/30 flex items-center justify-center text-[9px] font-bold text-[#f97316]">{a.username[0]?.toUpperCase()}</span>
                                                    ))}
                                                    {task.assignees.length > 3 && (<span className="text-[10px] text-white/30">+{task.assignees.length - 3}</span>)}
                                                </div>
                                            )}
                                            {isExpanded && (
